...
### #40 — Campaign forge & dark-fantasy presentation (cluster)

Make "Postavit kampaň s AI" (the forge) the centrepiece of the start
experience — a richer AI campaign builder wrapped in a deliberately atmospheric
UI. The generator already works end-to-end (`apps/server/src/vault/forge.ts`,
`POST /api/campaigns/forge`, `ForgeCampaign` in
`apps/web/src/components/StartMenu.tsx`, offline template fallback); this cluster
deepens *what* it produces and lifts *how* it looks and feels. Sub-items can ship
independently.

**Cross-cutting design intent (applies to every sub-item):**
- **Dark fantasy first.** Stay inside the palette + components in
  `apps/web/src/theme/tokens.css` (warm charcoal/stone neutrals; aged-gold /
  ember / blood accents; arcane mauve reserved for magic + active-actor). No
  emoji, no generic gradients, no AI-slop (#3).
- **Old-paper where it fits.** Authored/diegetic content — the forge brief, the
  generated campaign preview, character creation — should read as parchment
  (reuse the `.parchment` surface, ink text, deckled shadow), the same language
  as the character sheet. Chrome/HUD stays aged-metal `.panel`.
- **Tasteful motion/elevation.** Light, meaningful transitions on hover/reveal;
  always behind `prefers-reduced-motion` (the existing keyframes already gate on
  it). Effects where they *communicate*, not for their own sake.
- **Spacious, not sprawling.** Generous padding and clear hierarchy, but compact
  enough that the forge fits without endless scrolling — no stretched, empty
  layouts.

- **#40a — Deepen the AI campaign generator.** Grow the forge beyond the current
  locations + NPCs + one quest. Candidate depth: a richer authored quest (write
  a real `quests/*.md` once #19 lands, not just a lore note), starting
  encounters/bestiary picks grounded in the SRD index, faction/lore notes, a
  suggested starting party or pre-gen hero, per-location hooks and travel times,
  and a one-click "Mapa (AI)" (#37) kicked off automatically after a forge.
  Keep determinism: the LLM only drafts a structured spec that we validate with
  `zod` and write to a valid vault; the offline template must stay coherent.
  `apps/server/src/vault/forge.ts`, `apps/server/src/routes/game.ts`.
- **#40b — Ember particle background.** A subtle drifting-embers / floating-spark
  layer behind the play surface and the menus, evoking firelight in a dark hall.
  Implement as a single fixed, pointer-events-none canvas/WebGL layer well below
  the UI z-index (warm `--ember`/`--gold` motes, low opacity, gentle upward
  drift + flicker), cheap enough to idle at 60fps and fully disabled under
  `prefers-reduced-motion`. New `apps/web/src/components/EmberField.tsx`, mounted
  once in `App.tsx`.
- **#40c — Parchment treatment for forge & preview.** Re-skin `ForgeCampaign`
  (and the sibling "Nová kampaň" form) onto the `.parchment` surface with proper
  field styling, and add a campaign **preview/confirmation step** that shows the
  generated pitch/opening/locations/NPCs as an illuminated manuscript page before
  it's committed — with a light reveal animation. `StartMenu.tsx`, `tokens.css`.
- **#40d — Styling-consistency pass (no unstyled buttons).** Fix the bare,
  CSS-less buttons in the character-creation flow (`CharacterCreate.tsx`) and
  audit the menus for the same: every actionable control routes through a shared
  styled primitive (`.btn-gold` / a new secondary/ghost button class in
  `tokens.css`), so nothing renders as a default browser button. Establish the
  button as a reusable component to prevent regressions.
- **#40e — Layout rhythm.** Tighten spacing/hierarchy across the start menu and
  forge so the experience is airy but compact — consistent padding scale, aligned
  panel widths, and a clear primary action — feeding into #3/#7.
